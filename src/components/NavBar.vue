<template>
    <header class="max-w-full z-20">
        <nav class="flex border-b-2 border-b-primary2 items-center relative p-3 justify-between md:h-16 h-28 mx-auto md:px-4 container flex-wrap md:flex-nowrap">
            <!-- Logo -->
             ...
             <div :class="[
                ...
                <ul class="flex flex-col items-center space-y-5 md:flex-row md:space-x-5 md:space-y-0 font-medium">
                    <li v-for="item in Menu" :key="item.name">
                        <a :href="item.href" class="block transition hover:text-primary ease-linear text-2xl md:text-sm lg:text-lg text-white md:text-black" @click="scrollToSection(item.href)">{{ item.name }}</a>
                    </li>
                </ul>
             </div>
        </nav>
